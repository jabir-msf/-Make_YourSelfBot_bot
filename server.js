// --- ১. গ্লোবাল ক্র্যাশ প্রটেকশন (নোড.জেএস-এর যেকোনো আনহ্যান্ডেলড এরর আটকাতে এটি একদম উপরে রাখা আবশ্যক) ---
process.on("unhandledRejection", (reason, promise) => {
    console.error("⚠️ Unhandled Rejection at:", promise, "reason:", reason);
    // প্রসেস এক্সিট না করে সার্ভার সচল রাখবে
});

process.on("uncaughtException", (err) => {
    console.error("⚠️ Uncaught Exception caught:", err);
    // সার্ভার ক্র্যাশ করা থেকে রক্ষা করবে
});

// --- ২. আপনার মূল কোড এবং লাইব্রেরি সমূহ ---
...
// টেলিগ্রাম এডমিন নোটিফিকেশন হেল্পার ফাংশন
async function sendAdminNotification(text) {
    ...
    for (const adminId of ADMIN_IDS) {
        try {
            ...
            console.error(`Failed to send Telegram notification to admin ${adminId}:`, err);
        }
    }
}
...
app.post("/api/tasks/submit", async (req, res) => {
    ...
    try {
        ...
        // টাস্ক সাবমিশনের টেলিগ্রাম নোটিফিকেশন
        sendAdminNotification(`<b>📝 নতুন টাস্ক সাবমিশন!</b>\n\n👤 ইউজার আইডি: <code>${userId}</code>\n📋 টাস্কের নাম: ${taskName}\n💰 রেওয়ার্ড: ৳${parseFloat(amount || 0)}\n📁 ক্যাটাগরি: ${category}\n🔗 প্রুফ: ${proofUrl}`);
...
app.post("/api/admin/add-task", async (req, res) => {
    ...
    try {
        ...
        const finalReward = isNaN(parsedReward) ? 0.0 : parsedReward;
        ...
        if (error) {
            console.error("Supabase error during add-task:", error);
            throw error;
        }
        res.json({ success: true });
    } catch (err) { 
        console.error("API endpoint exception inside add-task:", err);
        res.status(500).json({ success: false, error: err.message || "ডাটাবেস কানেকশন ফেইল্ড হয়েছে।" }); 
    }
});
...
app.post("/api/buy-premium", async (req, res) => {
    ...
    try {
        ...
        // প্রিমিয়াম প্ল্যান ক্রয়ের টেলিগ্রাম নোটিফিকেশন
        sendAdminNotification(`<b>💎 নতুন মেম্বারশিপ ক্রয়!</b>\n\n👤 ইউজার আইডি: <code>${userId}</code>\n⭐ প্ল্যান: ${plan}\n💰 মূল্য: ৳${price}`);
...
app.post("/api/withdraw", async (req, res) => {
    ...
    try {
        ...
        // উইথড্রাল রিকোয়েস্টের টেলিগ্রাম নোটিফিকেশন
        sendAdminNotification(`<b>💸 নতুন উইথড্র রিকোয়েস্ট!</b>\n\n👤 ইউজার আইডি: <code>${userId}</code>\n💰 পরিমাণ: ৳${withdrawGrossAmount}\n💳 মেথড: ${method}\n📞 অ্যাকাউন্ট নং: ${accountNo}`);
...
// --- UPDATED ADMIN-TASKS WITH ERROR REPORTING ---
app.get("/api/admin-tasks/:userId/:category", async (req, res) => {
    const { userId, category } = req.params;
    try {
        const { data: allTasks, error: err1 } = await supabase
            .from('admin_tasks')
            .select('*')
            .eq('category', category);
            
        if (err1) {
            console.error("Supabase error fetching admin_tasks:", err1);
            throw err1;
        }

        const { data: userTasks, error: err2 } = await supabase
            .from('tasks')
            .select('task_name')
            .eq('user_id', userId);

        if (err2) {
            console.error("Supabase error fetching user tasks submissions:", err2);
            throw err2;
        }
        
        const submittedNames = userTasks ? userTasks.map(t => t.task_name) : [];
        const filteredTasks = allTasks ? allTasks.filter(task => !submittedNames.includes(task.title)) : [];
        
        res.json(filteredTasks);
    } catch (err) { 
        console.error("Exception during fetch admin-tasks:", err);
        res.json([]); 
    }
});
...
app.post("/api/add-money", async (req, res) => {
    ...
    try {
        ...
        // ডিপোজিট রিকোয়েস্টের টেলিগ্রাম নোটিফিকেশন
        sendAdminNotification(`<b>📥 নতুন ডিপোজিট রিকোয়েস্ট!</b>\n\n👤 ইউজার আইডি: <code>${userId}</code>\n💰 পরিমাণ: ৳${amount}\n💳 মেথড: ${method}\n🔑 TrxID: <code>${transactionId}</code>`);
...
app.post("/api/admin/delete-product", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ success: false, message: "পণ্য মুছতে ব্যর্থ হয়েছে।" });
    }
});
...
app.post("/api/buy-product", async (req, res) => {
    ...
    try {
        ...
        // প্রোডাক্ট অর্ডার ক্রয়ের টেলিগ্রাম নোটিফিকেশন
        sendAdminNotification(`<b>🛒 নতুন পণ্য অর্ডারের রিকোয়েস্ট!</b>\n\n👤 ইউজার আইডি: <code>${userId}</code>\n📦 পণ্য: ${product.title}\n💰 মূল্য: ৳${price}\n💳 পেমেন্ট মেথড: ${paymentMethod}${transactionId ? `\n🔑 TrxID: <code>${transactionId}</code>` : ''}`);
...
app.post("/api/recharge", async (req, res) => {
    ...
    try {
        ...
        // মোবাইল রিচার্জ রিকোয়েস্টের টেলিগ্রাম নোটিফিকেশন
        sendAdminNotification(`<b>📱 নতুন মোবাইল রিচার্জ রিকোয়েস্ট!</b>\n\n👤 ইউজার আইডি: <code>${userId}</code>\n📶 অপারেটর: ${operator}\n📞 নম্বর: ${number}\n💰 পরিমাণ: ৳${amount}`);
...
app.post("/api/buy-social-service", async (req, res) => {
    ...
    try {
        ...
        // সোশ্যাল সার্ভিস ক্রয়ের টেলিগ্রাম নোটিফিকেশন
        sendAdminNotification(`<b>📢 নতুন সোশ্যাল সার্ভিস অর্ডার!</b>\n\n👤 ইউজার আইডি: <code>${userId}</code>\n🛠 সার্ভিস: ${serviceName}\n🔗 লিঙ্ক: ${link}\n📦 পরিমাণ: ${quantity}\n💰 মোট মূল্য: ৳${totalCost.toFixed(2)}`);
...
// Root & Health Check: index.html এর পরিবর্তে সরাসরি dashboard.html রেন্ডার করে বাইপাস লজিক দেওয়া হয়েছে
app.get("/", (req, res) => res.sendFile(path.join(__dirname, "webapp", "dashboard.html")));
app.get("/health", (req, res) => res.json({ status: "online", app: "EarnBD-Pro" }));

const PORT = process.env.PORT || 3000;
// Render ডেপ্লয়মেন্টে সংযোগকারী পোর্টের কঠোরতা মেলাতে explicitly 0.0.0.0 তে লিসেন করা হচ্ছে
