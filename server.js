...
app.get("/api/user/:id", async (req, res) => {
    try {
        ...
        // যদি ডাটাবেস রিসেট করার কারণে ইউজার খুঁজে না পাওয়া যায়, তবে অটো-রেজিস্ট্রেশন করবে
        if (error || !data) {
...
app.post("/api/earn/limit-check", async (req, res) => {
    ...
    //Hourly ad limit ২০, এবং video limit ১০ সেট করা হয়েছে
...
app.post("/api/admin/action-withdraw", async (req, res) => {
    ...
    try {
        if (type === 'approve') {
            ...
            // --- রেফারেল বোনাস লজিক ---
            // যদি ব্যবহারকারীর কোনো রেফারকারী থাকে এবং এটিই তার প্রথম অনুমোদিত উত্তোলন হয়, তবে রেফারকারী বোনাস পাবে
            ...
            if (profile && profile.referrer_id) {
                ...
                if (prevApproved === 1) {
                    ...
                    if (referrer) {
                        ...
                        // রেফারকারীর ট্রানজেকশন রেকর্ড যোগ করা
...
app.post("/api/admin/add-task", async (req, res) => {
    const { title, reward, category, pass, link } = req.body;
    try {
...
// পাবলিক প্যাকেজ এন্ডপয়েন্ট (যাতে dynamic package card জেনারেট করা যায়)
