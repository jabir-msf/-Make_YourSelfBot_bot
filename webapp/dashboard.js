...
async function initDashboard() {
    ...
    try {
        ...
        // ডাইনামিক টায়ার এবং স্ট্যাটাস ব্যাজ আপডেট
        const userTier = data.tier || "সিলভার";
        const userStatus = data.is_banned ? "ব্যানড" : "সক্রিয়";
        
        const tierBadgeText = `<i class="fa-solid fa-crown"></i> ${userTier}`;
        const statusBadgeText = data.is_banned 
            ? `<i class="fa-solid fa-circle-xmark"></i> ${userStatus}` 
            : `<i class="fa-solid fa-circle-check"></i> ${userStatus}`;
        
        if (document.getElementById('userTier')) document.getElementById('userTier').innerHTML = tierBadgeText;
        if (document.getElementById('userStatus')) {
            const el = document.getElementById('userStatus');
            el.innerHTML = statusBadgeText;
            if (data.is_banned) {
                el.style.background = "rgba(231, 76, 60, 0.15)";
                el.style.color = "#e74c3c";
                el.style.borderColor = "rgba(231, 76, 60, 0.25)";
            }
        }
        if (document.getElementById('sideUserTier')) document.getElementById('sideUserTier').innerHTML = tierBadgeText;
        if (document.getElementById('sideUserStatus')) {
